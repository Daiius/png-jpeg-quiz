...
- **コミット・プッシュ・マージはユーザーから依頼された時のみ**行う。デフォルトブランチ上なら先にブランチを切る。
...
- 🔒 **コミットメッセージは 3 行以内を目安とする。** それを超える長さは
  ...
  コミットログは変更の索引であって、設計判断の置き場ではない。
- **マージは merge commit**（`gh pr merge --merge`）。**squash はしない**
  （1 コミット 1 論点で積むので、履歴をそのまま残す）。
- **レビュー bot**: マージ前に必ず 1 回は実行し、**指摘がゼロになるまで**「修正 → 再レビュー」を繰り返す。
  - 起動コマンド（bot 名）・完了判定・待機手順の正典は**ユーザースコープの `~/.claude/CLAUDE.md`**。
    bot は複数リポジトリ共通で名前も変わるため、ここには書かない。
  - bot 設定は [`.github/review-bot.json`](./.github/review-bot.json)。
    ⚠ **bot は既定ブランチの設定を読む。** PR head で書き換えてもその PR のレビューには効かない
    （`prd.paths` の**追加**だけは PR head からも拾われる）。
  - ⚠ **レビュー中に push すると bot は中断する**（無応答に見える）。投げる前に push を済ませ、
    **応答が返るまで push しない**。
  - 修正は**追加コミットで積む**（`--amend` + force push はしない。bot がコミット単位で追随できる）。
  - 的外れと感じる指摘は、独断で却下せずユーザーに相談する。
  - **bot が不調（無応答・エラー）のままマージしない。** タイムアウトしたら報告して指示を仰ぐ。
...
`.claude/local/`（gitignore 対象）が**存在する場合は**、その中のファイルも参照してよい。
リポジトリに残したくないローカル限定のルール・運用情報をそこに置く。

- **作業の続き**: `.claude/local/TASKS.md` が**あれば、セッション開始時に必ず読む**。
