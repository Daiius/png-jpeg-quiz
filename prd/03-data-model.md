...
**下段（セッション・ログ）だけが可変**。**守るべき実体**はこの線で分かれる——
上段はパイプラインの再実行と R2 から再生でき、失って困るのは下段だけ。
⚠ ただし**実運用のダンプは DB 全体を 1 スナップショットで取る**（[02](./02-architecture.md) §7）。
下段は上段への外部キーを持つため、部分ダンプでは整合の取れた復元ができない。
