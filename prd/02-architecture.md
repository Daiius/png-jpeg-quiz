...
- **色数ヒントはモード定義のオプション**（`standard-30` は減点 0.5 で許可。[06](./06-ranking.md) §7.4）。
  `score()` は「ヒント使用済みか」を ctx で受け取る純関数のまま変えない。
...
| `GET /api/session/:id` | セッション状態（得点・進行・**直前に回答済みの問題の結果**）。リロード復元用（[06](./06-ranking.md) §2.1） | **未回答の問題の情報**（入れてよいのは「現在の問題が配信済みか」の真偽値まで） |
| `GET /api/session/:id/question` | 現在の問題（`display` URL・寸法・カテゴリ）＋ **そのセッションの mode / profile** | **正解 / 両形式のバイト数 / png・jpeg の URL / 難易度の数値 / 得点の重み** |
| `POST /api/session/:id/hint` | 現在の問題の**色数レンジ**（2 段階）を開示。🔒 記録してから返す（[06](./06-ranking.md) §7.3） | **`color_count` の実数** / レンジ以外の属性 |
...
- 🔁 **`answer` は「同一回答の再送」に冪等**。直前に回答済みの問題への同一回答の再 POST には、
  採点をやり直さず**保存済みの結果**をそのまま返す。送信後に応答を失ったクライアントが、
  進行と得点を壊さずに安全にリトライできるようにするため（モバイル回線の瞬断は常態）。
  ⚠ **異なる回答の再送は 409**（回答は 1 回だけ。[03](./03-data-model.md) §7）。
- 🔁 **`hint` も再要求に冪等**（保存済みレンジを返し、二重減点しない）。回答済みの問題への要求は拒否。
  リロード復元（`GET /api/session/:id`）には**支払い済みの**ヒントの使用有無とレンジを含めてよい
  （「未回答の問題の情報」の唯一の例外。減点で対価済みのため。[06](./06-ranking.md) §7.3）。
